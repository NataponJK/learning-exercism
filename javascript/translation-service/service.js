...
export class TranslationService {
   ...
   */
  batch(texts) {
    if (!texts || texts.length === 0) return Promise.reject(new BatchIsEmpty());
  ...
  request(text) {
    return new Promise((resolve, reject) => {
      let attempts = 0;
      const attempt = (/** @type {any} */ error) => {
        if (!error) resolve();
        else if (attempts === 2) reject(error);
        else { attempts++; this.api.request(text, attempt); }
      };
      this.api.request(text, attempt);
    });
  }
